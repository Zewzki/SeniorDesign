...
def evaluateZone(circleList):
    ...
    sumX += circleList[len(circleList) - 1][0][0]
    sumY += circleList[len(circleList) - 1][0][1]
    ...
    for i in range(-3, 4):
        zoneList.append([(midPoint / 2) - avgRad + (i * (2 * avgRad)), (midPoint / 2) + avgRad + (i * (2 * avgRad))])
    ...
    if (time.time() - lastSuccessTime) > sleepOnSuccess:
        ...
        if framesSinceLastCircle > framesWithoutCircleThresh:
            ...
            if len(prevCircles) > 4:
                ...
                cv2.line(diff, (int(intercept), 0), (int(intercept + (slope * sHeight)), sHeight), green, 3)

                for z in zoneList:
                    cv2.line(frame, (int(z[0]), 0), (int(z[0]), sHeight), red, 1)
                    cv2.line(frame, (int(z[1]), 0), (int(z[1]), sHeight), red, 1)

                    cv2.line(diff, (int(z[0]), 0), (int(z[0]), sHeight), red, 1)
                    cv2.line(diff, (int(z[1]), 0), (int(z[1]), sHeight), red, 1)

                cv2.line(frame, (0, decisionLine), (sWidth, decisionLine), blue, 2)
                cv2.line(diff, (0, decisionLine), (sWidth, decisionLine), blue, 2)
